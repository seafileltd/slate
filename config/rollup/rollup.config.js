...
function configure(pkg, env, target) {
  ...
  const plugins = [
    ...
    typescript({
      abortOnError: false,
      tsconfig: `./packages/${getPkgName(pkg.name)}/tsconfig.json`,
    ...
    // modules by default.
    commonjs({
      exclude: [`packages/${getPkgName(pkg.name)}/src/**`],
    ...
    babel({
      runtimeHelpers: true,
      include: [`packages/${getPkgName(pkg.name)}/src/**`],
  ...
  if (isUmd) {
    return {
      ...
      output: {
        format: 'umd',
        file: `packages/${getPkgName(pkg.name)}/${
          isProd ? pkg.umdMin : pkg.umd
        }`,
        exports: 'named',
        name: startCase(getPkgName(pkg.name)).replace(/ /g, ''),
  ...
  if (isCommonJs) {
    return {
      ...
      output: [
        {
          file: `packages/${getPkgName(pkg.name)}/${pkg.main}`,
          format: 'cjs',
          exports: 'named',
          sourcemap: true,
        },
  ...
  if (isModule) {
    return {
      ...
      output: [
        {
          file: `packages/${getPkgName(pkg.name)}/${pkg.module}`,
          format: 'es',
          sourcemap: true,
        },
      ],
