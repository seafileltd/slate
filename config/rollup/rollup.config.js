...
function configure(pkg, env, target) {
  ...
  const input = `packages/${pkg.aliasname || pkg.name}/src/index.ts`
  const deps = []
  ...
  const plugins = [
    ...
    typescript({
      abortOnError: false,
      tsconfig: `./packages/${pkg.aliasname || pkg.name}/tsconfig.json`,
    ...
    // modules by default.
    commonjs({
      exclude: [`packages/${pkg.aliasname || pkg.name}/src/**`],
    ...
    babel({
      runtimeHelpers: true,
      include: [`packages/${pkg.aliasname || pkg.name}/src/**`],
      extensions: ['.js', '.ts'],
  ...
  if (isUmd) {
    return {
      ...
      output: {
        format: 'umd',
        file: `packages/${pkg.aliasname || pkg.name}/${isProd ? pkg.umdMin : pkg.umd}`,
        exports: 'named',
        name: startCase(pkg.aliasname || pkg.name).replace(/ /g, ''),
  ...
  if (isModule) {
    return {
      ...
      output: [
        {
          file: `packages/${pkg.aliasname || pkg.name}/${pkg.module}`,
          format: 'es',
          sourcemap: true,
        },
        {
          file: `packages/${pkg.aliasname || pkg.name}/${pkg.main}`,
          format: 'cjs',
          exports: 'named',
          sourcemap: true,
        },
